...
const highlightCode = (code: string): JSX.Element[] => {
  ...
  lines.forEach((line, li) => {
    ...
    for (const seg of merged) {
      ...
      parts.push(<span style={seg.style}>{line.slice(seg.start, seg.end)}</span>);
      idx = seg.end;
    }
  ...
  return (
    <Card
      style={{
        ...
        "box-shadow": hovered() ? "0 8px 24px rgba(114, 93, 66, 0.15)" : "none",
    ...
    >
      <img
        src={new URL(`./img/nook-phone/${props.feature.icon}`, import.meta.url).href}
        style={{
          width: "42px",
          height: "42px",
    ...
    <div ref={pageRef} style={{ ...S.page, overflow: "auto" }} onScroll={handleScroll}>
      ...
      <div style={S.hero}>
        <div style={isMobile() ? S.heroContentMobile : S.heroContent}>
          ...
          <div style={isMobile() ? { "text-align": "center" } : S.heroText}>
            ...
            <div
              style={{ ...S.heroActions, "justify-content": isMobile() ? "center" : "flex-start" }}
            >
              <Button type="primary" size="large" onClick={() => props.onNavigate?.("/button")}>
                开始使用 →
              </Button>
            </div>
          </div>
      ...
      <div style={{ ...S.section, padding: isMobile() ? "32px 16px" : "48px 40px" }}>
        ...
        <div style={S.features}>
          <For each={features}>{(f) => <FeatureCard feature={f} />}</For>
        </div>
      </div>
      ...
      <div style={{ ...S.section, padding: isMobile() ? "32px 16px" : "48px 40px" }}>
        ...
        <div style={S.compGrid}>
          <For each={components}>
            {(c) => (
              <Card style={S.compCard} onClick={() => props.onNavigate?.(`/${c.key}`)}>
      ...
      <div style={{ ...S.footer, padding: isMobile() ? "24px 16px" : "32px 40px" }}>
        <div style={S.footerLinks}>
          <span style={S.footerLink} onClick={() => props.onNavigate?.("/button")}>
            组件文档
          </span>
          <span
            style={S.footerLink}
            onClick={() =>
              window.open("https://github.com/zuisong/animal-island-ui-solid", "_blank")
            }
          >
            GitHub
          </span>
